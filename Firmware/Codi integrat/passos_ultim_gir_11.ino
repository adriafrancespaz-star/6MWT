  ...
  if (isRunning && deviceConnected) {
    ...
    if (millis() - lastSampleTime >= 20) {
      ...
      if (firstReading) {
        ...
        // Solo sumamos rotación si YA PASÓ el tiempo de enfriamiento (2 segundos)
        if (millis() - lastTurnTime > 2000) {
          ...
          // Si estamos en los 2 segundos de gracia tras un giro, 
          // clavamos el acumulador a 0. Así ignoramos el final del giro anterior.
          accumulatedRotation = 0;
        }
        // ------------------------------
      }
