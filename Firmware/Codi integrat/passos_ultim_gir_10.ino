...
class MyCharacteristicCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* pCharacteristic) {
    ...
    if (valor == "go") {
      ...
      // Reiniciar cronómetros de intervalos
      tiempoUltimoGiro = millis();
      lastTurnTime = millis(); // SOLUCIÓN: Evita que dispare un giro nada más arrancar
  ...
  if (isRunning && deviceConnected) {
    ...
    if (millis() - lastSampleTime >= 20) {
      ...
      if (firstReading) {
        ...
        lastHeading = currentHeading; // SOLUCIÓN: Siempre actualizamos el heading
        
        // SOLUCIÓN: Acumulamos todo. El movimiento natural de vaivén de la cadera
        // sumará en positivo y restará en negativo manteniéndose cerca del 0.
        ...
        if (abs(accumulatedRotation) >= TURN_THRESHOLD) {
          if (millis() - lastTurnTime > 2000) {  
            ...
            // SOLUCIÓN: Limpiamos el acumulador SOLO si el giro ha sido válido y procesado
            accumulatedRotation = 0; 
          }
        }
      }
