...
// Variables para podómetro (Ajuste fino)
unsigned long lastStepTime = 0;
const float STEP_THRESHOLD = 3.7;    // Subido un poco de 3.6 a 3.7
const int STEP_TIMEOUT = 430;        // Subido un poco de 400 a 430 para ocultar el rebote
      ...
      if (firstReading) {
        ...
        if (fabs(deltaAngle) > NOISE_THRESHOLD) {
          ...
          if (abs(accumulatedRotation) >= TURN_THRESHOLD) {
            if (millis() - lastTurnTime > 1500) {  // SUBIDO A 1500ms para estabilizar
              turnCount++;
              lastTurnTime = millis();
